...
public class AcessorioMaisUsado extends JFrame{
	Vector<Item> acessorio;
	public AcessorioMaisUsado(String titulo,Vector<Item> acessorio) {
        super(titulo);
        ...
        for(int i = 0; i < acessorio.size(); i++) {
			if(acessorio.get(i).getQntUso() > maiorUsoItem) {
				maisUsadoItem = acessorio.get(i);
				maiorUsoItem = acessorio.get(i).getQntUso();
			}
		}
        ImageIcon ft_maisrelogio = new ImageIcon("./imagens/relogio_maisusado.png");
        Image imagemRedim2 = ft_maisrelogio.getImage().getScaledInstance(150, 150, Image.SCALE_SMOOTH);
        ImageIcon maisacessorio_redim = new ImageIcon(imagemRedim2);
        if(maisUsadoItem != null) {
	        // foto e tipo
	        JLabel ft_parteCima = new JLabel(maisacessorio_redim, SwingConstants.CENTER);
