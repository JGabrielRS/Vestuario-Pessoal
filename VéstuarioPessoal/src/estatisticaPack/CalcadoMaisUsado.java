...
public class CalcadoMaisUsado extends JFrame{
	Vector<Item> calcado;
	public CalcadoMaisUsado(String titulo,Vector<Item> calcado) {
        super(titulo);
        ...
        for(int i = 0; i < calcado.size(); i++) {
			if(calcado.get(i).getQntUso() > maiorUsoItem) {
				maisUsadoItem = calcado.get(i);
				maiorUsoItem = calcado.get(i).getQntUso();
			}
		}
