...
public class ParteIntimaMaisUsada extends JFrame{
	Vector<Item> parteIntima;
	public ParteIntimaMaisUsada(String titulo,Vector<Item> parteIntima) {
        super(titulo);
        ...
        for(int i = 0; i < parteIntima.size(); i++) {
			if(parteIntima.get(i).getQntUso() > maiorUsoItem) {
				maisUsadoItem = parteIntima.get(i);
				maiorUsoItem = parteIntima.get(i).getQntUso();
			}
		}
        ImageIcon ft_maiscueca = new ImageIcon("./imagens/cueca_maisusada.png");
        Image imagemRedim2 = ft_maiscueca.getImage().getScaledInstance(150, 150, Image.SCALE_SMOOTH);
        ImageIcon maiscueca_redim = new ImageIcon(imagemRedim2);
        if(maisUsadoItem != null) {
	        // foto e tipo
	        JLabel ft_parteCima = new JLabel(maiscueca_redim, SwingConstants.CENTER);
	        ...
	        JLabel nome_parteCima = new JLabel("Não Existe Parte Intima mais Usada!", SwingConstants.CENTER);
	        nome_parteCima.setPreferredSize(new Dimension(200,50));
	        nome_parteCima.setBorder(BorderFactory.createLineBorder(Color.BLACK));
	        gbc.gridy = 0;
	        gbc.gridx = 0;
