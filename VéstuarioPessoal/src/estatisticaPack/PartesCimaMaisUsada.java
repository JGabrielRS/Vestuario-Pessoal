...
public class PartesCimaMaisUsada extends JFrame{
	Vector<Item> parteCima;
	public PartesCimaMaisUsada(String titulo,Vector<Item> parteCima) {
        super(titulo);
        ...
        for(int i = 0; i < parteCima.size(); i++) {
			if(parteCima.get(i).getQntUso() > maiorUsoItem) {
				maisUsadoItem = parteCima.get(i);
				maiorUsoItem = parteCima.get(i).getQntUso();
			}
		}
