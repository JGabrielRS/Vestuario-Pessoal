...
public class CriarLook extends JFrame{
	...
	private JTextArea adicionar_nome;
	...
	public CriarLook(String titulo, Vector<Item> itens, Vector<Look> looks, Vector<Item> parteCima, Vector<Item> parteBaixo, Vector<Item> calcado, Vector<Item> acessorio, Vector<Item> parteIntima) {
		super(titulo);
        this.itens = itens;
        ...
        adicionar_nome = new JTextArea();
        adicionar_nome.setPreferredSize(new Dimension(100, 50));
        adicionar_nome.setBorder(BorderFactory.createLineBorder(Color.BLACK));
        adicionar_nome.setAlignmentX(SwingConstants.CENTER);
        adicionar_nome.setText("ex: Farda");
        
        // Componentes
        JLabel parteCimaComp = new JLabel("parte de cima:", SwingConstants.CENTER);
        parteCimaComp.setPreferredSize(new Dimension(200, 50)); 
        ...
        salvar.addActionListener(e -> salvarAlteracoes(looks));
        ...
        gbc.gridx = 0;
        panelEsquerdo.add(nome, gbc);
        
        gbc.gridy = 2;
        
        gbc.gridx = 0;
        panelEsquerdo.add(adicionar_nome, gbc);
	...
	private void salvarAlteracoes(Vector<Look> looks) {
		try {
			String novoNome = adicionar_nome.getText();
			Item novaParteCima = parteCimatemp;
			Item novaParteBaixo = parteBaixotemp;
			Item novoCalcado = calcadotemp;
			Item novoAcessorio = acessoriotemp;
			Item novaParteIntima = parteIntimatemp;
            int novaQntUso = Integer.parseInt(adicionar_qntUso.getText());
            Look looknovo = new Look(novoNome,novaParteCima,novaParteBaixo,novoCalcado,novoAcessorio,novaParteIntima,novaQntUso);
			looks.add(looknovo);
			...
			JOptionPane.showMessageDialog(this, "Ocorreu um erro ao criar o look :"+e.getMessage());
		}
	}
}
